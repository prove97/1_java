...
//		int[] copy = origin; // copy에 origin의 주소값을 복사
...
//		// 왜? origin과 copy의 같은 곳을 참조하고 있기 때문(즉, 메모리 주소값이 같다)
//		// 얕은 복사 : 주소값을 복사
...
//		System.out.println();
		...
		int[] origin = {1, 2, 3, 4, 5};
		int[] copy = new int[10]; // 0~9번 인덱스. 모든 값은 0으로 초기화 되어 있음
		
		//System.arraycopy(원본 배열, 복사 시작할 인덱스, 복사본 배열, 복사본 배열의 복사 시작 인덱스, 복사할 갯수);
		...
		// 주소값에 따라 값이 달라짐 (깊은 복사)
		// -> 각자 다른 배열을 참조하고 있음
		// -> 즉, 배열 수정 시 서로에게 영향 주지 않음
...
//		
//		int[] origin = {1, 2, 3, 4, 5};
//		int[] copy = origin.clone(); // 주소값이 다르게 복사
//		
//		for(int i=0; i <copy.length; i++) {
//			System.out.print(copy[i] + " ");
//		}
