...
public class TCP_Server {
	 ...
	 * - 데이터를 교화하기에 앞서 서버, 클라이언트 연결이 되어야한다.(서버가 먼저 실행되어 클라이언트에 요청을 기다림)
	 * - 신뢰성있는 데이터 전달가능
	 * 
	 * *Socket
	 * - 프로세스간의 통신을 담당
	...
	public static void main(String[] args) {
		...
		// 1) 포트번호 지정(서버 측에서 몇 번 포트로 통로를 열겠냐 라는 의미)
		int port = 3000;
		ServerSocket server;
		
		try {
			// 2) ServerSocket 객체생성시 포트 결합(Bind)
			server = new ServerSocket(port);
			
			// 3) 클라이언트로부터 접속요청이 올때까지 대기
			System.out.println("클라이언트 요청을 기다리고 있습니다.");
			
			// 4) 연결요청이 오면 => 요청 수락 후 클라이언트와 통신할 수 있는 서버측 소켓 객체를 생성
			Socket socket = server.accept();
			System.out.println(socket.getInetAddress().getHostAddress() + "가 연결을 요청함..");
		
			// 5) 클라이언트와 입출력 기반스트림 생성(바이트스트림 밖에 안됨)
			// 6) 보조스트림을 통한 성능개선
