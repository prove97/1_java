 ...
 * - 프로그램 실행 시 발생되는 예외들
 * 
 * * RuntimeException 후손들 (상속받은 클래스들)
 ...
 */
	public void method01() {
		/* ArithmeticException : 나누기 연산시 0으로 나눠질 때 발생되는 에외 */
		...
		// 예외 자체가 발생이 안되게끔 if문으로 조건 검사(예외처리는 아님)
//		int result = 0;
//		if(num2 != 0) {
//			result = num1 / num2;
//		}
...
//		System.out.println("나누기 결과 : " + result);
	...
	public void method02() {
		...
		// ArrayIndexOutOfBoundsException : 배열의 부적절한 인덱스로 제시하면서 접근했을 때 발생하는 오류
		// NegativeArraySizeException : 배열의 할당 과정에서 크기를 읍수로 지정하면 발생하는 예외
		...
		System.out.println("100번째 인덱스값 : " + arr[100]);
			...
			System.out.println("배열의 크기를 음수로 설정할 수 없습니다.");
		}
	...
	public void method03() throws IOException {
		...
		// Scanner와 같이 키보드로 값을 입력받을 수 있는 객체(단, 문자열로 읽어들여진다.)
		BufferedReader br = new BufferedReader(new InputStreamReader(System.in)); //ctrl + shift + o (자동 import)
		
//		// 1. try~catch문 : 여기 이자리에서 곧바로 예외를 처리하겠다.
//		// try{} : 예외가 발생할 가능성이 있는 코드를 작성
//		// catch(발생될 예외 클래스 매개변수) {} : try 구문내에서 에외가 발생했을 경우 어떻게 처리할지를 정의
//		try {
...
//		}
		...
		// 2. throws : 지금 이 자리에서 예외를 처리하지않고 현재 이 메소드를 호출한 곳으로 예외처리를 떠넘기겠다.
		String str = br.readLine();
