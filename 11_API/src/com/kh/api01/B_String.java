...
public class B_String {
	public void method01() {
		...
		// String클래스에 toString메소드는 이미 오버라이딩 되어있음(시제 담긴 문자열을 반환하도록)

		System.out.println(str1 == str2); // false (주소값 비교)
		System.out.println(str1.equals(str2)); // true
		// String클래스에 equals메소드는 이미 오버라이딩 되어있음(주소값 비교가 아닌 담김 문자열을 비교하도로)
		
		System.out.println(str1.hashCode());
		System.out.println(str2.hashCode());
		// String클래스의 hashCode메소드 이미 오버라이딩 되어있다.(주소값이 아닌 문자열을 가지고 만들도록)
	...
	public void method02() {
		...
		//    문자열에서 전달받은 index 위치의 문자 하나를 추출해서 리턴한다.
		...
		//    CharSequence(인터페이스)는 String(클래스)을 implements
		...
		//    => 문자열의 beginIndex위치에서부터 끝까지 문자열 추출해서 리턴
		//    문자열.substring(int beginIndex, int endIndex) : String
		//    => 문자열의 beginIndex위치에서부터 (endIndex - 1)위치까지 문자열을 추출해서 리턴
		System.out.println(str1.substring(6));
		System.out.println(str1.substring(0, 5));
		
		// 7. 문자열.replace(char oldChar, char newChar) : String
		//    문자열에서의 oldChar문자을 newChar문자로 변환한 새 문자열 리턴
		String str4 = str1.replace('l', 'c');
		String str = str1.replace("Hello", "Hi"); // CharSequance 객체 매개변수
...
//		char ch = new java.util.Scanner(System.in).next().toUpperCase().charAt(0); // 'N', 'Y'
		...
		// 9. 문자열.trim() : String
		//    문자열의 앞 뒤 공백을 제거시킨 새 문자열을 리턴(문자열 사이에 있는 공백은 X)
		String str7 = "    JA    VA    ";
		System.out.println(str7.trim());
		
		// 10. 문자열.tocharArray() : char[] => 문자열을 문자 배열로 변환
...
//		}  
		...
		// 11. static valueOf(char[] data) : String => 문자 배열을 다시 문자열로 변환
	...
	public void method03() {
		...
		for(int i = 0; i < arr.length; i++) {
			System.out.println(i + "배열 : " + arr[i]);
		}

//		int i = 0;
//		for(String st : arr) { // 두번째 반복문 출력방법
//			System.out.println(i + "배열 : " + st);
//			i++;
//		}
		...
		StringTokenizer stn = new StringTokenizer(str, ","); //(기본값은 공백 " ")
		
		System.out.println(stn.countTokens());

		int length = stn.countTokens();
		for(int i = 0; i < length; i++) {  // coutnTokens()의 값은 점점 줄어든다.
			System.out.println(stn.nextToken());
		}
		
		System.out.println(stn.countTokens()); 
	
		System.out.println(stn.nextToken()); // NoSuchElementException발생(더 이상 꺼낼 토큰이 없음)
		...
		// 방법 2.
		while(stn.hasMoreTokens()) { // 토큰 가진게 있으면 true, 없으면 false
			System.out.println(stn.nextToken());
		}
