...
public class MapRun {
	...
	public static void main(String[] args) {
		...
		// => 객체를 추가하고자 할때 put메소드를 이용(key + value 세트로 담아야 함)
		
		// 1. put(K key, V value) : Map에 key, value세트로 추가시켜주는 메소드
		...
		// 저장되는 순서 유지 안됨. value값이 중복되어도 키값이 중복되지 않는다면 문제없이 저장됨
		...
		hm.replace("먹태깡", new Snack("엄청 짠맛" , 600)); // 일치하는 키값이 없으면 변화없음
		System.out.println(hm);
		...
		// Iterator 반복자 이용 => 안됨
		// 바로Iterator를 호출할 수 없음 -> Iterator는 List계열 또는 Set계열에서만 가능하기 때문
		
		// Map ->  Set 변경해주는 메소드를 제공한다
		
		// 1. keySet() 이용하는 방법
		Set keySet = hm.keySet();
		System.out.println("set" +keySet); // key들만 넣어줌
		...
		while(itEntry.hasNext()) {
			Entry entry = (Entry)itEntry.next(); // Entry객체 안에 key, value값이 다 담겨있다.
		...
		// Map 게열이기 때문에 put 메소드를 이용해서 key + value 세트로 담는다.
//		prop.put("다이제", new Snack("초코맛", 1000));
//		prop.put("칸쵸", new Snack("단맛", 500));
//		prop.put("새우깡", new Snack("짠맛", 400));
		prop.setProperty("list", "ArrayList");
		prop.setProperty("set", "HashSet");
		prop.setProperty("Map", "HashMap");
		prop.setProperty("Map", "Properties");
		
		System.out.println(prop); // 저장 순서 유지안됨, key값 중복시 덮어씌움
		
		// Properties는 주로 Properties에 담긴 것들을 파일로 출력 또는 입력받아 올때 사용함
		// 즉, Properties에서 제공하는 store(), load() 등의 메소드를 사용하기 위해 사용
		
		try {
			//store : Properties에 담겨있는 key-value값들을 파일형태로 저장할 때 사용하는 메소드
			prop.store(new FileOutputStream("test.properties"), "propertiesTest");
			// ClassCastException 
			// 내부적으로 store 실행 시 properties에 담겨있는 key+value 세트들을 String으로 형변환해서 출력함
			
			//storeToXML : Properties에 담겨있는 key-value값들을 xml 파일형태로 저장할 때 사용하는 메소드
		...
		/*
		 * Properties를 사용하는 이유
		 * 프로그램 상에 필요한 기본 환경설정(서버의 ip주소, dbms경로 ...) 관련한 구문을 기술할 것이다.
		 * => 모두 문자열이기 때문에 개발자가 아닌 일반인 관리자가 해당 문서를 수정하기 쉽지 않다.
		 * 
